...
from datasets import DATES
from models import adapt_model
...
def main(args):
    ...
    data_loader_val = torch.utils.data.DataLoader(
        dataset_val, batch_size=args.batch_size, sampler=test_sampler, num_workers=args.workers, pin_memory=True
    )
    data_loader_test = torch.utils.data.DataLoader(
        dataset_test, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=True
    )
    ...
    if args.log_wandb:
        ...
        if args.specialist is not None:
            wandb.run.name = f"{args.model}_regression-{args.regression}_specialist-{args.specialist}"
        else:
            wandb.run.name = f"{args.model}_regression-{args.regression}"
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--specialist", default=None, type=str, help="specialist: train a specialist model")
